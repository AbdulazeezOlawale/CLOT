...
export const useCustomFlatListHook = (): TUseCustomFlatListHook => {
  ...
  const styles: ICustomFlatListStyles = {
    header: {
      marginBottom: heights.sticky + heights.topList + heights.header, // <-- In order for the list to be under other elements
    },
    stickyElement: {
      left: 0,
    ...
    topElement: {
      left: 0,
      marginTop: heights.header + heights.topList + heights.sticky, // <-- In order for the list to be under other elements
      position: "absolute",
      right: 0,
      transform: [
        {
